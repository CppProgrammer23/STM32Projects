...
#include "STM32F413H-Discovery/stm32f413h_discovery_ts.h"
...
int main(void)
{
  ...
  BSP_TS_InitEx(BSP_LCD_GetXSize(), BSP_LCD_GetYSize(), LCD_ORIENTATION_LANDSCAPE_ROT180);
  BSP_TS_ITConfig();
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, LD5_ARD_Pin|LCD_CTP_RST_Pin|LCD_TE_Pin|WIFI_WKUP_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : LD5_ARD_Pin LCD_CTP_RST_Pin LCD_TE_Pin WIFI_WKUP_Pin */
  GPIO_InitStruct.Pin = LD5_ARD_Pin|LCD_CTP_RST_Pin|LCD_TE_Pin|WIFI_WKUP_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);
...
void DrawScreen(void)
{
	...
	BSP_LCD_DisplayStringAt(0, 90, (uint8_t*)"Welcome to this demo", CENTER_MODE);
	HAL_Delay(2000);
	BSP_LCD_Clear(LCD_COLOR_WHITE);
	BSP_LCD_SetFont(&Font12);
	BSP_LCD_DisplayStringAt(0, 15, (uint8_t*)"Toggle the LEDs w/ colored button", CENTER_MODE);
	BSP_LCD_SetTextColor(LCD_COLOR_RED);
	BSP_LCD_DrawCircle(60, 160, 30);
	BSP_LCD_FillCircle(60, 160, 30);
	BSP_LCD_SetTextColor(LCD_COLOR_GREEN);
	BSP_LCD_DrawCircle(180, 160, 30);
	BSP_LCD_FillCircle(180, 160, 30);
}
TS_StateTypeDef TS_S = {0};
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	if(GPIO_Pin == GPIO_PIN_0)
		return;
	BSP_TS_GetState(&TS_S);
	if(TS_S.touchX[0]<195 && TS_S.touchX[0]>165 && TS_S.touchY[0]<175 && TS_S.touchY[0]>145)
		HAL_GPIO_TogglePin(LED1_RED_GPIO_Port, LED1_RED_Pin);
	else if(TS_S.touchX[0]<75 && TS_S.touchX[0]>45 && TS_S.touchY[0]<175 && TS_S.touchY[0]>145)
