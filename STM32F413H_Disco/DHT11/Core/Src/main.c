...
#include "../../Drivers/BSP/STM32F413H-Discovery/stm32f413h_discovery_lcd.h"
...
/* USER CODE BEGIN PFP */
/*static void setInput(GPIO_TypeDef*,uint16_t);
static void setOutput(GPIO_TypeDef*,uint16_t);*/
static uint8_t DHT11_Init(void);
static DHT11 DHT11_ReadValues();

/*static float FromCelcuisToFahr(float);*/
static uint8_t checkP(void);
...
{
	setOutput(GPIOA, GPIO_PIN_4);
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, 0);
	delay_us(18000);
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, 1);
	delay_us(20);
	setInput(GPIOA,GPIO_PIN_4);
}
uint8_t DHT11Response(void)
{
	uint8_t res=0;
	/*get response*/
	delay_us(40);
	...
	while(HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_4)); //wait to pin goes low
	return res;
}
...
DHT11 DHT11_ReadValues()
{
	...
	sprintf(temp,"%i.%iC",t1,t2);
	BSP_LCD_DisplayStringAt(20, 120, (uint8_t*)hum, CENTER_MODE);
	BSP_LCD_DisplayStringAt(20, 90, (uint8_t*)temp, CENTER_MODE);
	dht11.humidity=atoff(hum);
	dht11.temperature=atoff(temp);
	return dht11;
}
uint8_t checkP(void)
{
	return readDataDHT11();
}
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	...
	DHT11_Init();
	DHT11_ReadValues();
	if(checkParity==checkP())
		BSP_LCD_DisplayStringAt(20, 150, (uint8_t*)"Data are correct", CENTER_MODE);
	else
		BSP_LCD_DisplayStringAt(20, 150, (uint8_t*)"Data are not correct", CENTER_MODE);
}
/* USER CODE END 4 */
